...
[assembly: Dependency(typeof(FilesHelper))]
namespace DosingApp.Droid.Helpers
{
...
/*        public static async Task CopyFileAsync(Context context, global::Android.Net.Uri uri, string destinationPath)
        {
            Stream sourceStream = context.ContentResolver.OpenInputStream(uri);

            using (sourceStream)
            using (FileStream destinationStream = System.IO.File.Create(destinationPath))
            {
